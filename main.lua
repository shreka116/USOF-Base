...
-- Create model
local model, photometric_loss, smoothness_loss, criterion = models.setup(opt, checkpoint)
...
local trainer = Trainer(model, photometric_loss, smoothness_loss, criterion, opt, optimState)
...
print(string.format(' * Finished average EPE: %1.3f  %% of err. Pixels: %1.3f', avgEPE,errPixel  ))
end
